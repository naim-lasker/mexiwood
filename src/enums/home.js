export const furnitureItems = [
    {
        catagory: 'Langmore 36',
        img: require('../images/svg/ven/ven2.svg'),
        imgType: 'langmore_36',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'Malibu 36',
        img: require('../images/svg/ven/ven3.svg'),
        imgType: 'malibu_36',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'langmore 60',
        img: require('../images/svg/ven/ven1.svg'),
        imgType: 'langmore_60',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'Malibu 60',
        img: require('../images/svg/ven/ven4.svg'),
        imgType: 'malibu_60',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'Prestige 36',
        img: require('../images/svg/ven/ven5.svg'),
        imgType: 'prestige 36',
        detailsList:
            [       
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'Prestige 60',
        img: require('../images/svg/ven/ven6.svg'),
        imgType: 'prestige_60',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'Soho 36',
        img: require('../images/svg/ven/ven7.svg'),
        imgType: 'soho_36',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    },
    {
        catagory: 'Soho 60',
        img: require('../images/svg/ven/ven8.svg'),
        imgType: 'soho_60',
        detailsList:
            [
                { value: 'Light Gray Finish' },
                { value: 'Brushed nickel finish hardware' },
                { value: 'Natural 1 in.carrara white marble countertop and backsplash' },
                { value: 'Top pre-drilled for 8 in. Widespread faucet' },
                { value: 'White vitreous china rectangular undermount sink' },
                { value: 'Adjustable height levelers' },
                { value: 'Faucet not included' },
                { value: 'Hardware color: Brushed Nickel' },
                { value: 'Primary Wood: Poplar and Secondary Wood:MDF' },
                { value: 'Shape:Rectangular' },
                { value: 'Number of Doors:2' },
                { value: 'Number of Shelves:1' },
                { value: 'Vanity Cabinet Size:37-Inch' }
            ],
        tabData:
            [
                {
                    title: 'Details',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Dimensions',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Specifications',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
                {
                    title: 'Shipping and Returns',
                    description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip'
                },
            ]
    }
]